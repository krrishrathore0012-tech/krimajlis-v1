"""
paper_trade_v8.py — KRIMAJLIS Acquisition-Grade Paper Trading Simulation
══════════════════════════════════════════════════════════════════════════
Starting capital : ₹1,00,00,000 (₹1 Cr)
...
Costs            : 0.10% slippage + 0.05% brokerage per side (0.30% RT)
...
Currency         : All ETF returns in USD; portfolio denominated in INR
...
CAPITAL_INR    = 10_000_000   # ₹1 Cr
START_DATE     = '2021-01-01'
END_DATE       = '2025-12-31'
SLIPPAGE       = 0.0010       # 0.10% per side
BROKERAGE      = 0.0005       # 0.05% per side
COST_RT        = (SLIPPAGE + BROKERAGE) * 2   # 0.30% round-trip
...
def generate_markdown(meta, summary, yearly, mc, families, trades):
    ...
    md = f"""# KRIMAJLIS v8 — Acquisition-Grade Paper Trading Simulation
...
KRIMAJLIS v8 paper trading simulation achieved a **{summary["total_return_pct"]}% total return** (₹{pnl_cr:.2f} Cr absolute P&L) on ₹1 Cr starting capital over the 2021–2025 clean window, compounding at **{summary["cagr_pct"]}% CAGR**. The engine deployed capital across {summary["total_trades"]} regime-gated signal firings with a **{summary["win_rate_pct"]}% win rate**, a Sharpe ratio of **{summary["sharpe_ratio"]}**, and a maximum drawdown of **{summary["max_drawdown_pct"]}%**, demonstrating institutional-grade risk-adjusted returns attributable to three empirically validated causal relationships in precious metals and sector ETF markets.
...
*Document generated by KRIMAJLIS paper_trade_v8.py | v8 engine | {meta["generated_at"]}*
...
def main():
    ...
    print("=" * 70)
    print("KRIMAJLIS v8 — ACQUISITION-GRADE PAPER TRADING SIMULATION")
    ...
    meta = {
        ...
        "methodology":        "Quarter-Kelly (0.25×edge), 0.30% round-trip cost, 4% cash yield, VIX regime-gated",
    ...
    json_path = "results/krimajlis_paper_trade_v8.json"
    ...
    md_path = "results/krimajlis_paper_trade_v8_report.md"
    ...
    gates = [
        ("JSON file exists and is valid JSON",        json_ok and json_valid),
        ("MD report exists with all 6 sections",      md_ok and md_sects),
        ...
        ("Caveats section present in MD report",       caveats_ok),
    ]
    all_pass = True
